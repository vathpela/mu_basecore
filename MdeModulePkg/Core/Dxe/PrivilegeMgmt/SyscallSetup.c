  ...
  )
{
  ...
  Edx = ((LONG_DS_R0_PH << 16) | LONG_CS_R0) & 0xFFFFFFFF;
  ...
  )
{
  ...
  // MU_CHANGE: DXE_SUPV: This APIC ID may not be the real cpu index...
  Status = SetupCpl0MsrStar (GetApicId());
  ...
  )
{
  ...
  Status = RestoreCpl0MsrStar (GetApicId ());
  ASSERT_EFI_ERROR (Status);
  return Status;
}
...
{
  return GetThisCpl3Stack (GetApicId ());
}
  ...
  )
{
  ...
  return (EFI_PHYSICAL_ADDRESS) (UINTN) ((UINTN)mDxeCpl3StackArrayBase + mDxeStackSize * ((CpuIndex) + 1) - sizeof (UINTN));
}
